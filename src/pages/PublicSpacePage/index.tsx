    ...
    (async () => {
      ...
      if (did) {
        ...
        if (pUser && pUser.did) {
          ...
          spaces = await SpaceService.getSpaceByNames(pUser, [spaceName]);
        }
      } else {
        spaces = await SpaceService.getCommunitySpaceByNames([spaceName]);
      }

      setSpaceProfile(spaces.length > 0 ? spaces[0] : []);
      setLoading(false);
    })();
