...
const CreateIdentityPage: React.FC<InferMappedProps> = ({
  eProps,
  ...props
}: InferMappedProps) => {
  /**
    ...
    async function generateDid() {
      ...
      if (savedMnemonic) {
        ...
        localStorage.setItem(
          'mnemonic',
          JSON.stringify(mnemonicObject.mnemonic.split(' '))
        );
    ...
    <IonPage className={style['createidentitypage']}>
      <ClearlyMeContent>
        ...
        <div className={style['main-container']}>
          ...
          <div
            className={
              encrypt ? style['warning-light'] : style['warning-emphasis']
            }
          >
            <p className={style['text']}>
              These are your security words (like a password).
              <br />
              <br />
              Lose these words and you will lose the identity. Keep them written
              down, in order, and safe. Write them down now.
            </p>
          </div>

          <IonList>
            <IonItem className={style['consent']}>
              <IonLabel className={style['text']}>
                Encrypt and save security words to my vault
              </IonLabel>
              <IonCheckbox
                checked={encrypt}
                slot='start'
                className={style['checkbox-label']}
                onIonChange={(e) => setEncrypt(!encrypt)}
              ></IonCheckbox>
            </IonItem>
          </IonList>
          <br />
 ...
 */
const withInjectedMode = injector(CreateIdentityPage, {
  key: NameSpace,
  reducer,
  saga,
});
