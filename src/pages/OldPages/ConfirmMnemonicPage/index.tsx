...
const ConfirmMnemonicPage: React.FC<InferMappedProps> = ({
  eProps,
  ...props
}: InferMappedProps) => {
  ...
  const showButton = () => {
    if (nextIndex < 12 || isValid(mnemonic))
      return (
        <div className='buttons-collection'>
          <ButtonDefault
            style={{ width: '49%' }}
            className={
              nextIndex <= 0 || isValid(mnemonic) ? style['btn-disabled'] : ''
            }
            title='Undo'
            onClick={undo}
            disabled={nextIndex <= 0 || isValid(mnemonic)}
          >
            Undo
          </ButtonDefault>
          <ButtonDefault
            href='/publish'
            style={{ width: '49%' }}
            className={isValid(mnemonic) ? '' : style['btn-disabled']}
            title='Continue'
    ...
    return (
      <ButtonDefault title='Start over' onClick={reset}>
        Start over
      </ButtonDefault>
    );
  };
  ...
  const MnemonicWord = function (mnemonic: any) {
    if (mnemonic.hide) {
      return (
        <div
          className={style['mnemonic'] + ' ' + style['hide']}
          onClick={mnemonic.click}
          style={{ width: '30%', display: 'inline-block', margin: '0.2em' }}
        ></div>
      );
    } else {
      return (
        <div
          className={style['mnemonic'] + ' ' + style['shuffled']}
          onClick={mnemonic.click}
        >
          {mnemonic.title}
        </div>
    ...
    return (
      <>
        <div className={style['mnemonic']}>
          <span
            className={mnemonic.title ? style['number-dark'] : style['number']}
          >
            {mnemonic.number}
          </span>
          {mnemonic.title}
      ...
      <ClearlyMeContent>
        ...
        <div className={style['main-container']}>
          ...
          <div className={style['mnemonic-wrapper']}>
            {item.map((number: any, key: any) => (
              <MnemonicInput
                key={`mnemonic-key-${key}`}
                number={item[number - 1]}
                title={words[number - 1]}
              />
            ))}
          </div>
          ...
          <div
            style={{
              display:
                nextIndex >= 12 && !isValid(mnemonic) ? 'inline-block' : 'none',
            }}
          >
 ...
 */
const withInjectedMode = injector(ConfirmMnemonicPage, {
  key: NameSpace,
  reducer,
  saga,
});
