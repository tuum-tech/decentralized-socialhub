...
const { InsertExecutable } = require('@elastosfoundation/hive-js-sdk/typings');
const {
  AggregatedExecutable
} = require('@elastosfoundation/hive-js-sdk/typings');
const { find } = require('lodash');

let setupCommunitySpaces = async hiveClient => {
...
let setupAssets = async hiveClient => {
...
let setupSpaces = async hiveClient => {
  await hiveClient.Database.createCollection('spaces');

  await hiveClient.Scripting.registerScript(
    'get_all_spaces',
    new FindExecutable('get_all_spaces', 'spaces', null, null).setOutput(true),
    undefined,
    true,
    true
  );
...
let setupComments = async hiveClient => {
...
let setupFollowers = async hiveClient => {
...
let setupVerifications = async hiveClient => {
...
let setupUsers = async hiveClient => {
...
let setupFeedbacks = async hiveClient => {
...
let run = async () => {
  try {
    ...
    setupSpaces(hiveClient);
    setupComments(hiveClient);
    setupFollowers(hiveClient);
    setupVerifications(hiveClient);
    setupUsers(hiveClient);
    setupFeedbacks(hiveClient);
